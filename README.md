...
* It might use different version of jackson json library which will create havoc in your dependencies. You might end up making duplicate project just to have this thing running (not always this easy drop in). Actually lot of the promises are not so great as they look.
...
* It support only hibernate 4 instead of 5. Which has different default naming strategy for fields.
* Writing custom naming strategy will be partially ignored as well. So part of the tool work and part will not.
...
* As I mentioned before every field in the form needs to exist in database, so if you want have something different (like type password twice), so you would take the value only when both of them match can't be done without dirty workarounds.
...
* It looked like maintained project when I started with it, now after months I have feeling it's bit abadoned and not maintained anymore (last commit is from Sep 2015). Which woudn't be problem if this tool woudn't have so much problems and be behind with some versions of libraries.
